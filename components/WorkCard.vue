<template>
    <NuxtLink :to="`/works/${work.id}`">
        <div :class="`max-w-md mx-auto rounded-md shadow-lg overflow-hidden md:max-w-2xl mb-3 ` + (work.color ? `bg-${work.color}` : getRandomBgColor())">
            <div class="md:flex">
                ...
                <div class="p-8 h-48 md:pt-12 flex flex-col justify-center">
                    ...
                    <p class="mt-2 text-slate-100 text-sm">{{ work.preview ?? truncate(work.challenge, 100) }}</p>
                </div>
            </div>
        </div>
    </NuxtLink>
